...
export default function CartPage({cartItems} : { cartItems: CartItem[]}) {
  ...
  const validCartItems = cartItems.filter(item => item.products && item.products.id);
          ...
          <div className="space-y-4">
            <ul role="list" className="space-y-4">
              {validCartItems.map((item) => {
                ...
                if (!item.products) {
                  console.warn('⚠️ [CartPage] Skipping item with missing products:', item.id);
                  return null;
                }
            ...
            <div className="mt-8 border-t border-gray-700 pt-8">
              <div className="flex justify-between text-lg font-medium text-white mb-4">
                <p>Subtotal</p>
                <p>${validCartItems.reduce((sum, item) => sum + (item.products?.price || 0), 0).toFixed(2)}</p>
              </div>
